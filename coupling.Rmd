...
if (file.exists("CLUSTER")) {
  ...
  library(doMPI)
  cl <- startMPIcluster(ncpu,verbose=TRUE)
}
```
...
By setting $\rho = 0$ in the gravity model, we obtain mean-field model.


The negative log likelihood function for the mean-field model is:
```{r meanfield-like,purl=TRUE}
likfnMeanField <- function (theta, phi, psi, tau1, tau2) {
  theta <- exp(theta)
  phi <- exp(phi)
  Q <- (N^tau2)*dd
  iota <- (N^tau1)*(theta*crossprod(Q,ylag)+phi)
...
```{r meanfield-profile-2D,cache=FALSE,purl=TRUE}
...
results %>%
  ggplot(aes(x=tau1,y=tau2,z=loglik,
    fill=ifelse(loglik>max(loglik)-2000,loglik,NA)))+
  geom_tile(color=NA)+geom_contour(bins=100,color='white')+
  geom_point(color='red',shape="x",size=3,data=mle.meanfield)+
  geom_hline(color='red',size=0.2,linetype=2,data=mle.meanfield,aes(yintercept=tau2))+
  geom_vline(color='red',size=0.2,linetype=2,data=mle.meanfield,aes(xintercept=tau1))+
  labs(x=expression(tau[1]),y=expression(tau[2]),fill=expression(log(L)))
```

```{r meanfield-pairsplot,fig.height=6.83}
...
By setting $\tau_1 = \tau_2 = 0$ in the gravity model, we obtain a diffusion model, which couples cities in a way that depends on ly on the distance between them.
...
```{r diffusion-pairsplot,fig.height=6.83}
...
mle.grav %<>% summaryGravity()
mle.xia %<>% summaryXia()
mle.meanfield %<>% summaryMeanField()
mle.diffusion %<>% summaryDiffusion()
mle.compdest %<>% summaryCompDest()
mle.stouffer %<>% summaryStouffer()
mle.stouffer1 %<>% summaryStouffer1()
mle.radiation %<>% summaryRadiation()
mle.radiation1 %<>% summaryRadiation1()
...
list(
  ...
  rename(c(theta="$\\log{\\theta}$",cHat="$\\hat{c}$",phi="$\\log{\\phi}$",rho="$\\rho$",
    ...
    loglik="$\\ell$",qaic="QAIC",delta.qaic="$\\Delta\\!\\mathrm{QAIC}$")) %>%
  kable(digits=3)
```
...
require(ncf)
spatial.plot(imp$long, imp$lat, imp$CD-imp$GR, main="CD-GR", ctr=FALSE)
spatial.plot(imp$long, imp$lat, imp$CD-imp$SV, main="CD-SV", ctr=FALSE)
```


```{r maps,message=FALSE,results="hide"}
library(rgdal)
library(aakmisc)

readOGR("EngWal.shp") -> gb
...
data.frame(
  ...
  dplyr::mutate(erate=export/N,irate=import/N) -> impexp

startTunnel()
getQuery("select * from coords") %>% arrange(town) -> coords
stopTunnel()

join(impexp,coords,by="town") -> impexp

impexp %>%
  dplyr::select(town,model,erate,irate) %>%
  dplyr::filter(model %in% c("GR","ST","CD","SV")) %>%
  tidyr::gather(variable,value,erate,irate) %>%
  ...
  mutate(
    m1=ordered(m1,levels=c("GR","ST","CD","SV")),
    m2=ordered(m2,levels=c("GR","ST","CD","SV"))
  ) %>%
  dplyr::filter(m1 > m2) %>% 
...
foreach (m1=1:4) %:%
  foreach (m2=1:4) %do% {
    if (m1 > m2) {
      model1 <- c("GR","ST","CD","SV")[m1]
      model2 <- c("GR","ST","CD","SV")[m2]
      ied %>%
        dplyr::filter(m1==model1&m2==model2) %>%
      ...
      gb %>%
        ...
        geom_point(
          data=d,
          # aes(x=long,y=lat,color=value,alpha=abs(value)),
          aes(x=long,y=lat,color=sign(value)*(abs(value)^(1/4))),
          size=1
        )+
        # expand_limits(color=c(-0.0014,0.00073),alpha=c(0.0014))+
        guides(alpha=FALSE)+
        scale_color_gradient2(mid="gray90")+
        labs(title="differences in predicted import rates",
          subtitle=sprintf("%s - %s",model1,model2),
          x="",y="")+
...
spatialLiks %>% name_rows() %>% dplyr::filter(town!=.rownames) %>% nrow() 
...
gb %>%
  ...
  geom_point(
    data=spatialLiks,
    aes(x=long,y=lat,color=SV-CD,alpha=abs(SV-CD)),
    size=1
  )+
  # expand_limits(color=c(-0.0014,0.00073),alpha=c(0.0014))+
  guides(alpha=FALSE)+
  scale_color_gradient2(mid="gray90")+
  labs(title="differences in log likelihood",
    subtitle=sprintf("SV - CD"),
    x="",y="")+
...
gb %>%
  ...
  geom_point(
    data=spatialLiks,
    aes(x=long,y=lat,color=CD-GR,alpha=abs(CD-GR)),
    size=1
  )+
  # expand_limits(color=c(-0.0014,0.00073),alpha=c(0.0014))+
  guides(alpha=FALSE)+
...
gb %>%
  ...
  geom_point(
    data=spatialLiks,
    aes(x=long,y=lat,color=SV-GR,alpha=abs(SV-GR)),
    size=1
  )+
  # expand_limits(color=c(-0.0014,0.00073),alpha=c(0.0014))+
  guides(alpha=FALSE)+
  scale_color_gradient2(mid="gray90")+
  labs(title="differences in log likelihood",
    subtitle=sprintf("SV - GR"),
    x="",y="")+
  theme(axis.text=element_blank())
```

```{r network,purl=TRUE,cache=FALSE,eval=FALSE}
list(
  SV=melt(SVmat),
  ST=melt(STmat),
  GR=melt(GRmat),
  ...
  ldply() %>% 
  dplyr::rename(model=.id,dest=Var1,src=Var2) %>% 
  dplyr::filter(dest!=src) %>%
  dplyr::left_join(
           coords,
           by=c("src"="town")
         ) %>%
  dplyr::rename(src.lat=lat,src.long=long) %>%
  dplyr::left_join(
           coords %>% mutate(N=N[town]),
           by=c("dest"="town")
         ) %>%
  dplyr::rename(dest.lat=lat,dest.long=long) -> mats

mats %>%
  dplyr::filter(model=="SV") %>%
  dplyr::group_by(dest) %>%
  dplyr::filter(rank(-value)<=100) %>%
  dplyr::ungroup() -> matslim
...
gb %>%
  ...
  geom_segment(
    data=matslim,
    mapping=aes(
      x=src.long,y=src.lat,
      xend=dest.long,yend=dest.lat,
      color=log10(value)
    )
  )+
  transition_states(N,transition_length=0.1,state_length=0.2)+
#  guides(color=FALSE)+
  expand_limits(color=c(0,log10(3.27e4)))+
  geom_map(aes(x=long,y=lat,map_id=id),map=fortify(gb),fill=NA,color="black",size=0.3)+
  coord_map()+
  labs(x="",y="",title="Stouffer variant model",subtitle="{closest_state}")+
  theme(axis.text=element_blank()) -> pls

animate(pls,nframes=10000)

```

```{bash,results="hide",eval=FALSE}
ffmpeg -y -r 5 -i figure/coupling-network-1.gif network.webm
ffmpeg -y -r 5 -i figure/coupling-network-1.gif network.mp4
```
...
```{r ccs-residuals,eval=FALSE}
dat %>%
  ddply(~town,summarize,
    efrac=mean(cases==0),
    N=mean(pop)) %>%
  dplyr::filter(rank(-N)>6) %>% 
  mutate(logit.ext=log(efrac/(1-efrac)))  -> dat1

fit <- lm(logit.ext~log(N),data=dat1)
dat1$resid <- residuals(fit)
...
data.frame(
  ...
  name_rows() %>%
  dplyr::rename(town=.rownames) %>%
  dplyr::left_join(dat1,by="town") -> dat1

dat1 %>%
  tidyr::gather(model,imports,SV,ST,GR,CD) %>%
  ggplot(aes(x=imports,y=resid))+
  geom_point()+
  geom_smooth()+
  scale_x_log10()+
  facet_wrap(~model)

imp$ext=log(ext[,2]/(1-ext[,2]))
imp$ext[!is.finite(imp$ext)]=NA
imp$resext=NA
imp$resext[is.finite(imp$ext)]=resid(lm(imp$ext~I(log(imp$N))))
cor(imp[,c(1:4, 8:9)], use="pairwise")
```
