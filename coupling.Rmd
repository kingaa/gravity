...
```{r gravity-like}
likfnGravity <- function (theta, phi, rho, tau1, tau2) {
  theta <- exp(theta)
  phi <- exp(phi)
  rho <- exp(rho)
  Q <- (N^tau2)*(dd^rho)
...
par <- list(theta=exp(-8), phi = 1e-15, rho=1, tau1=0.5, tau2=0.5)
with(par,likfnGravity(log(theta),log(phi),log(rho),tau1,tau2))
...
with(as.list(coef(test)),c(exp(c(theta=theta,phi=phi,rho=rho)),tau1=tau1,tau2=tau2))
...
bake(file="gravity.rds",{
  ...
  foreach(start=iter(grd,"row"),
  ...
  ) %do% try(
    {
      mle2(likfnGravity,
           method="Nelder-Mead",
           start=as.list(start[c("rho","theta","phi")]),
...
pairs(~loglik+theta+phi+rho+tau1+tau2,
      data=results,
...
```{r stouffer1-like}
likfnStouffer1 <- function (theta, tau1, tau2) {
  theta <- exp(theta)
  iota <- theta*(N^tau1)*((rr^tau2)%*%ylag)
  lambda <- betaS*iota[relevant]^alpha
  -sum(dpois(x=obs,lambda=lambda,log=TRUE))
...
```{r stouffer1-test,purl=FALSE}
par <- list(theta=0, tau1=0.5, tau2=0.5)
with(par,likfnStouffer1(theta,tau1,tau2))
...
results %>%
    subset(loglik==max(loglik)) -> mle.stouffer1
```
...
list(gravity=mle.grav,
  ...
  ldply(.id="model") %>%
  arrange(-loglik) %>%
  rename(c(theta="log theta",rho="log rho")) %>%
  kable(digits=3)
```
...
```{r iota-plot,fig.height=6.83,purl=FALSE}
#Gravity
bake(file="gravity-test.rds",{
    tic <- Sys.time()
    test <- mle2(likfnGravity,
...
```{r expectations from spatial models,purl=FALSE,eval=F}
...
```{r k-means dendrogram of model predictions,purl=FALSE,eval=F}
