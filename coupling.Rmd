...
bake(file="gravity.rds",{
  ...
  foreach(start=iter(grd,"row"),
          .errorhandling="pass",
          .export=c("likfnGravity","N","dd","ylag","betaS",
            "alpha","relevant","obs"),
          .inorder=FALSE,
          .packages="bbmle"
  ) %dopar% try(
...
likfnCompDest <- function (theta, rho, psi, tau1, tau2, delta) {
  theta <- exp(theta)
  rho <- exp(rho)
  Q <- (N^tau2)*(dd^rho)
...
bake(file="compdest.rds",{
  ...
  foreach(start=iter(grd,"row"),
  ...
  ) %dopar% try(
    {
      mle2(likfnCompDest,
           method="Nelder-Mead",
           start=as.list(start[c("rho","theta","delta","psi")]),
           fixed=as.list(start[c("tau1","tau2")]), 
           control=list(trace=0,maxit=10000),
           skip.hessian=TRUE) -> fit
...
pairs(~loglik+theta+rho+delta+psi+tau1+tau2,
      data=results,
      subset=loglik>max(loglik)-10000,cex=0.5)
```
...
list(gravity=mle.grav,
  ...
  ldply(.id="model") %>%
  arrange(-loglik) %>%
  rename(c(theta="log theta",phi="log phi",psi="log psi",rho="log rho")) %>%
  kable(digits=3)
```
