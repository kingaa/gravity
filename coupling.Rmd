...
gb %>% 
  ...
  coord_map()+
  geom_point(
    data=ied %>% arrange(abs(irate)),
    aes(x=long,y=lat,color=irate,alpha=abs(irate)),
    size=1
    )+
  ...
  labs(title="differences in predicted import rates",
    color=expression(Delta~i),
    size=expression(Delta~i),
    x="",y="")+
  facet_wrap(~m1+m2,nrow=2)+
...
gb %>% 
  ...
  coord_map()+
  geom_point(
    data=ied %>% arrange(abs(erate)),
    aes(x=long,y=lat,color=erate,alpha=abs(erate)),
    size=1
    )+
  # scale_size_area(max_size=2)+
  guides(alpha=FALSE)+
  scale_color_gradient2(mid="gray90")+
  labs(title="differences in predicted export rates",
    color=expression(Delta~i),
    size=expression(Delta~i),
    x="",y="")+
  facet_wrap(~m1+m2,nrow=2)+
