---
title: "Spatial coupling in measles"
output:
  html_document:
    theme: default
    toc: yes
...
sigma1dev <- function (dat, sigma) {
  ...
  fit <- glm(log(I)~-1+seas+log(Ilag)+I(1/Ilag)+offset(-log(Ps)+log(slag)),
...
tsirfit <- function (dat) {
  glm(log(I)~-1+seas+log(Ilag)+I(1/Ilag)+offset(-log(Ps)+log(Slag)),
...
tsirs %>% 
  ...
  ddply(~variable, function (d) {
    ...
    },.parallel=TRUE) %>% dcast(town~variable) -> tsircoef

dat %<>% join(tsircoef,by="town")
...
bake(file="distances.rds",{
  ...
  startTunnel()
  getQuery("select * from coords") -> coords
  stopTunnel()
...
We assume that
$X_{i}(t+1) \sim \dist{Bernoulli}{e^{-\lambda_i(t)}}$.
...
**Do we need to worry about under-reporting?**
```{r gravity-pre}
ySel <- t(MuSel)/PuSel
S <- t(SuSel)
beta <- t(exp(betaSel))
...
fadeout <- ySel==0
relevant <- which(fadeout[,-ncol(fadeout)])
obs <- fadeout[,-1][relevant]
```

The negative log likelihood function for the gravity model is:
```{r gravity-like,eval=FALSE}
likfn <- function (theta, rho, a, b) {
  theta <- exp(theta)
  rho <- exp(rho)
  tau1 <- a+b
  tau2 <- a-b
  Q <- (PuSel^tau2)*(dd^rho)
  iotaSel <- theta*(PuSel^tau1)*crossprod(Q,ySel)
...
```{r gravity-test,eval=FALSE}
par <- c(theta=-3, rho=0, a=0.5, b=0.5)
with(as.list(par),likfn(theta,rho,a,b))
...
test <- mle2(likfn,
             method="BFGS",
             start=as.list(as.list(coef(test))),
             control=list(trace=4,maxit=1000))
toc <- Sys.time()
toc-tic
...
with(as.list(coef(test)),c(exp(c(theta=theta,rho=rho)),tau1=a+b,tau2=a-b))
```

```{r gravity-test-2,eval=FALSE}
vcov(test)
cov2cor(vcov(test))
```

```{r gravity-test-3,eval=FALSE}
ctest<-confint(test); ctest
ptest<-profile(test)
```
...
bake(file="results.rds",{
  grd <- expand.grid(
    tau1=seq(0.1, 2, by=.05), 
    tau2=seq(-1, 1, by=.05),
    rho=log(1.0),
    theta=log(0.05)
  )
  ...
  foreach(start=iter(grd,"row"),
          .errorhandling="pass",
          .inorder=FALSE,
          .options.mpi=list(seed=95886868),
          .packages="bbmle"
  ) %do% try(
    {
      mle2(likfn,
  ...
  toc <- Sys.time()
  print(toc-tic)  ## 9 hr on 100 processors
...
results %>%
  ggplot(aes(x=tau1,y=tau2,z=loglik,fill=loglik))+
  geom_tile(color=NA)+geom_contour(bins=100)+
  labs(x=expression(tau[1]),y=expression(tau[2]))
```


```{r eval=FALSE}
##########
#source('t1t2t2015.dump')

require(locfit)

grd<-log(expand.grid(tau1=seq(0.1, 2, by=.05), tau2=seq(0.1, 2, by=.05)))

data=na.omit(data.frame(llik=t1t2$llik, theta=exp(t1t2$coef[,1]), rho=exp(t1t2$coef[,2]), tau1=exp(grd[,1]), tau2=exp(grd[,2])))

prof=locfit(llik~lp(tau1, tau2, nn=0.1, h=0.1), data=data)
plot(prof)
...
sub=data[data$tau1<1&data$tau2<1,]

prof=locfit(llik~lp(tau1, tau2, nn=0.1, h=0.1), data=sub)
plot(prof)
points(sub[which.max(sub$llik),4],sub[which.max(sub$llik),5])


#######prof code below#######
require(bbmle)

par <- c(theta=-15, rho=0)
...
#t1t2<-list(llik=rep(NA, 1521), coef=matrix(NA, ncol=2, nrow=1521))

for(j in 601:1521){
    cat(date(), '\n')
    test<-mle2(likfn, start=list(theta=-15, rho=0), fixed=list(tau1=grd[j,1], tau2=grd[j,2]), control=list(trace=4), skip.hessian=T)
    cat(j, '\n')
    t1t2$llik[j]<-logLik(test)[1]
    t1t2$coef[j,]<-coef(test)[1:2]
    dump('t1t2', 't1t2t2015.dump')
    cat(date(), '\n')
}
```
