...
fadeout <- which(ylag==0&slag>=0)
obs <- obs[fadeout]>0
betaS <- beta[fadeout]*slag[fadeout]
```
...
likfn <- function (theta, rho, tau1, tau2) {
  ...
  prob <- exp(-betaS*iota[fadeout]^alpha)
  -sum(log(ifelse(obs,1-prob,prob)))
}
```
...
with(par,likfn(theta,rho,tau1,tau2))
...
```{r gravity-test-2,purl=FALSE}
vcov(test)
cov2cor(vcov(test))
```

```{r gravity-profile-2D,eval=FALSE}
bake(file="gravity.rds",{
  ...
  foreach(start=iter(grd,"row"),
          .errorhandling="pass",
          .inorder=FALSE,
          .options.mpi=list(seed=95886868,chunkSize=4),
          .packages="bbmle"
  ) %do% try(
    {
      mle2(likfn,
