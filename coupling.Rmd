...
```{r radiation-like}
likfnRadiation <- function (theta, psi, phi) {
  theta <- exp(theta)
  phi <- exp(phi)
...
```{r radiation1-like}
likfnRadiation1 <- function (theta, psi, phi) {
  theta <- exp(theta)
  phi <- exp(phi)
  iota <- theta*(rr%*%ylag)+phi*N
  lambda <- betaS*iota[relevant]^alpha
...
```{r latex_table,include=FALSE,purl=FALSE}
library(xtable)
library(tibble)
tab %>%
...
impexp %>%
  ...
  dplyr::filter(model %in% c("SV","CD","G","RV")) %>%
  dplyr::mutate(
           model=factor(model,levels=c("SV","RV","CD","G"))
         ) %>%
...
impexp %>%
  ...
  dplyr::filter(model %in% c("RV","G","CD","SV")) %>%
  ...
  mutate(
    m1=ordered(m1,levels=c("RV","G","CD","SV")),
    m2=ordered(m2,levels=c("RV","G","CD","SV"))
  ) %>%
  dplyr::filter(m1 > m2) %>% 
...
expand.grid(m2=1:4,m1=1:4) %>%
  dplyr::filter(m1<m2) %>%
  dplyr::mutate(
           model1=c("SV","CD","G","RV")[m1],
           model2=c("SV","CD","G","RV")[m2]
         ) -> comps
...
```{r spatial_fadeouts,results="hide",purl=FALSE}
fadeouts %>%
